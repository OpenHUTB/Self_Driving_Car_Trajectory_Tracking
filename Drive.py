...
#     world = client.get_world()
#
#     settings, weather = make_carla_settings(args)
#     world.apply_settings(settings)
#     world.set_weather(weather)
#
#     blueprint_library = world.get_blueprint_library()
#     vehicle_bp = blueprint_library.filter('vehicle.audi.tt')[0]
#     spawn_points = world.get_map().get_spawn_points()
#     vehicle = world.try_spawn_actor(vehicle_bp, spawn_points[PLAYER_START_INDEX])
#
#     #############################################
#     # Load Configurations
#     #############################################
...
#                              color='deepskyblue')
#
#     trajectory_fig.add_graph("lookahead_path", window_size=INTERP_MAX_POINTS_PLOT,
...
#                              marker="D", color='red', markertext="Finish",
...
#                                 window_size=TOTAL_EPISODE_FRAMES, color='gray')
#     forward_speed_fig.add_graph("forward_speed", label="forward_speed",
#                                 window_size=TOTAL_EPISODE_FRAMES, color='deepskyblue')
#
#     throttle_fig = lp_params.plot_new_dynamic_figure(title="Throttle (%)")
#     throttle_fig.add_graph("throttle", label="throttle",
#                            window_size=TOTAL_EPISODE_FRAMES, color='deepskyblue')
#
#     brake_fig = lp_params.plot_new_dynamic_figure(title="Brake (%)")
#     brake_fig.add_graph("brake", label="brake",
#                         window_size=TOTAL_EPISODE_FRAMES, color='deepskyblue')
#
#     steer_fig = lp_params.plot_new_dynamic_figure(title="Steering Angle (deg)")
#     steer_fig.add_graph("steer", label="steer",
#                         window_size=TOTAL_EPISODE_FRAMES, color='deepskyblue')
#
...
#             trajectory_fig.update("lookahead_path", -new_waypoints_np[path_indices.astype(int), 0], -new_waypoints_np[path_indices.astype(int), 1], new_colour='green')
#
#             forward_speed_fig.roll("forward_speed", current_timestamp, current_speed*3.6)
#             if args.longitudinal_controller == 'PID':
#                 forward_speed_fig.roll("reference_signal", current_timestamp, controller._desired_speed*3.6)
#
